...
from filescanio.console import colorize_report, spinning, wants_colour
...
def _spinning(label: str) -> AbstractContextManager[None]:
    """Spin on stderr while a handler runs, only when a person is watching."""
    return spinning(_stream("stderr"), label)
...
def _emit(
    ...
    if (
        ...
        and wants_colour(stdout)
    ):
        payload = colorize_report(payload.decode()).encode()
